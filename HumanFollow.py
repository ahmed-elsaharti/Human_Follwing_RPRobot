...
#
# I added the integration with the RaspberryPi and the linear mapping equation to compensate for the low refresh rate
...
while True:
    ...
    for i in range(len(scores)):
        ...
        # Reset the x and y position of the detection to the center 

        xp=50
        xy=50

        # Enter if detection is a person
        ...
        if ((scores[i] > min_conf_threshold) and (scores[i] <= 1.0) and labels[int(classes[i])]=="person"):
            ...
            xp=int(x*100/imW)
            yp=int(y*100/imH)
            print('Person at %d , %d' % (xp,yp))
            direction=50-xp
            if xp<=45:
                turnpwm=14*abs(direction)+800
                set_speed(turnpwm,turnpwm)
                turn_left()
                time.sleep(0.1)
                stopcar()
            if xp>=55:
                ...
                stopcar()
            if (xp>45 and xp<55):
                percentage=int(area*100/framearea)
                print("area is % d" % (int(area*100/framearea)))
                if percentage<25:
                    turnpwm=1500
                    set_speed(turnpwm,turnpwm)
                    go_forward()
